# First unzip the shapenet_data.tar.gz folder in your ada as described in readme
...
def core_processing():
    ...
    for idx in range( name_num ):
        ...
        populate( voxel_model_256, voxel_model_b, voxel_model_bi )
        ...
        dim_voxel = 64
        voxel_model_64 = different_resolutions( dim_voxel, voxel_model_256 )
        ...
        for i in range( 3, dim_voxel - 3 ):
            ...
            for j in range( 3, dim_voxel - 3 ):
                ...
                for k in range( 3, dim_voxel - 3 ):
                    if batch_size_counter >= batch_size: break
                    batch_size_counter = sampling_points( batch_size_counter, i, j, k, sample_points, sample_values,
                                                          voxel_model_64, voxel_model_64_flag )
        if (batch_size_counter >= batch_size):
            ...
            for i in range( 0, dim_voxel, 2 ):
                for j in range( 0, dim_voxel, 2 ):
                    for k in range( 0, dim_voxel, 2 ):
                        filled_flag = False
                        for (i0, j0, k0) in [(i, j, k), (i + 1, j, k), (i, j + 1, k), (i + 1, j + 1, k),
                                             (i, j, k + 1), (i + 1, j, k + 1), (i, j + 1, k + 1),
                                             (i + 1, j + 1, k + 1)]:
            ...
            slot_filling( batch_size, batch_size_counter, dim_voxel, sample_points, sample_values, voxel_model_64,
                          voxel_model_64_flag )
        ...
        dim_voxel = 32
        voxel_model_32 = different_resolutions( dim_voxel, voxel_model_256 )
        ...
        for i in range( 3, dim_voxel - 3 ):
            ...
            for j in range( 3, dim_voxel - 3 ):
                ...
                for k in range( 3, dim_voxel - 3 ):
                    if batch_size_counter >= batch_size: break
                    batch_size_counter = sampling_points( batch_size_counter, i, j, k, sample_points, sample_values,
                                            voxel_model_32, voxel_model_32_flag )
        if (batch_size_counter >= batch_size):
            ...
            slot_filling( batch_size, batch_size_counter, dim_voxel, sample_points, sample_values, voxel_model_32,
                          voxel_model_32_flag )
        ...
        dim_voxel = 16
        voxel_model_16 = different_resolutions( dim_voxel, voxel_model_256 )
    ...
    return exceed_32, exceed_64


def slot_filling(batch_size, batch_size_counter, dim_voxel, sample_points, sample_values, voxel_model_32,
                 voxel_model_32_flag):
...
def sampling_points(batch_size_counter, i, j, k, sample_points, sample_values, voxel_model_64, voxel_model_64_flag):
...
def different_resolutions(dim_voxel, voxel_model_256):
    ...
    return voxel_model_64


def populate(voxel_model_256, voxel_model_b, voxel_model_bi):
    for i in range( 16 ):
        for j in range( 16 ):
            for k in range( 16 ):
...
if __name__ == "__main__":
    ...
    for kkk in range( len( class_name_list ) ):
        ...
        # dir of voxel models
        voxel_input = "../shapenet/blenderRenderPreprocess/" + class_name + "/"
        ...
        exceed_32, exceed_64 = core_processing()
